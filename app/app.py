...
def running_model(df, pipeline, model, threshold=0.5):
    ...
    if "Churn" in df.columns:
        ...
        churn_test = [("Yes" if i<df.shape[0]/2 else "No") for i in range(df.shape[0])]
    ...
    return {"df": df, "df_processed": df_,"churn_in": churn_in, "accuracy_score": accuracy, "f1_score": f1, "recall_score": recall}
...
def main():
    ...
    col1.metric("Total quantity churn prediction", f'{df[df["Churn_prediction"]==1].shape[0]} ({df[df["Churn_prediction"]==1].shape[0]/df.shape[0]*100:.1f}%)')
    col2.metric("Total monthly charges by churn prediction", f'${df_processed["MonthlyCharges"].sum():,.1f}')
    col3.metric("Total charges by churn prediction", f'${df_processed["TotalCharges"].sum():,.1f}')
    if churn_in:
    ...
    data_export = df[["customerID", "gender", "Churn_prediction","Churn_prediction_proba"]].sort_values("Churn_prediction_proba", ascending=False)
